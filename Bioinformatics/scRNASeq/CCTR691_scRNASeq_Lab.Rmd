...
> 1) there are a lot more cells, 2) slightly low reads per cell 3) tsne was a blod
...
```{r setup}
knitr::opts_knit$set(root.dir = "C:/Users/oajen/OneDrive/Documents/coding/CCTR691_scRNASeq")
```
...
> counts, orig.ident, feature, active assay
...
> 2575 cells with 19612 unique genes
...
> The buldges in the feature and counts moved up and is no longer clustered down on the bottom of the graph
...
> The x axis is the principle component and the y axis is is something too small to read (probably genes)
...
> there is a decrease in color intensity, so decrease in the relatedness
...
>you loose some amount of grouping and your plot changes shape but if you decrease the overall shape stays the same
...
> you loose a lot of information and it becomes very linear

What happens when you use more dimensions?

```{r}

whim_filtered <- RunTSNE(whim_filtered, dims = 1:8)
DimPlot(whim_filtered, reduction = "tsne")

```

> <it doesn't really look any different if you increase the dimensions
...
> It shows the expression change in each gene in each cluster
